...
<cfif subAction eq "update">
	...
	<cfif !error>
		...
		<cflocation url="../html/admin.cfm?p=promoCodes" addtoken="false">
	<cfelse>
		<cflocation url="../html/admin.cfm?p=promoCodes&subAction=edit&promoCodeID=#form.promoCodeID#&error=true" addtoken="false">
	</cfif>
...
<cfif subAction eq "save">
	...
	<cfif !error>
		...
		<cflocation url="../html/admin.cfm?p=promoCodes&subAction=view" addtoken="false">
	<cfelse>
		<cfset subAction = 'add'>
	</cfif>
...
<cfquery name="dataQ" 
    ...
    and pc.vessel_type_id = #val(CONS_YMI_Motor_Promo_ID)#
	and pc.site_id = #session.thirdgenas.siteid#
		...
		<td style="border-left:none;border-right: 1px solid ##DCDBDB;" colspan="2"> 
        	...
        	or FindNoCase("ADMIN-3",CONST_userPrivilege) gt 0 >
				<cfif subAction neq 'add'>
					<a title="Add Promo Code" href="../html/admin.cfm?p=promoCodes&subAction=add" class="btn btn-default btn-sm pull-right">
		...
		<tr>
        	...
        	or FindNoCase("ADMIN-3",CONST_userPrivilege) gt 0 >
					<cfif okToEditStartDate or okToEditEndDate> 
						<a title="Edit Promo Code" href="../html/admin.cfm?p=promoCodes&subAction=edit&promoCodeID=#dataQ.id#">
...
<cfif subAction eq "edit">
	...
	<!--- EDIT --->
	<form name="editpromoCode" action="../html/admin.cfm?p=promoCodes&subAction=update"  method="post">
...
<cfelseif subAction eq "add">
	...
	<!--- ADD --->
	<form name="addHullForm" action="../html/admin.cfm?p=promoCodes&subAction=save"  method="post">
	<tr>
		<td style="display:none;">       
			<!--- vessel type --->
			<select name="vessel_type_id" class="form-control" style="display:none;">
