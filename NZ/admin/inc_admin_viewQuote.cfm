...
<cfif getData.quoteStatus eq CONST_BQ_QuoteStatus_stage3_LID or getData.quoteStatus eq CONST_BQ_QuoteStatus_stage4_LID>
    <!--- do nothing --->
<cfelse>
...
<cfif IsDefined("session.thirdgenAS.userid") and session.thirdgenAS.userid neq 1> <!--- only Super Admin can view all fields --->
    <cfif FileExists(ExpandPath("../custom/mod_motorcycleQuoteForm.cfm"))>
        <cfmodule template="../custom/mod_motorcycleQuoteForm.cfm" formDataId="#attributes.formDataId#" incValidation=false>
    </cfif>
</cfif>