...
<cfparam name="attributes.pdfTemplate" default="#application.BASE_FOLDER#adminMarine\pdf_template\YMINZ_Marine_PayByMonth_form.pdf">
...
<cfif attributes.formdataid gt 0 and StructCount(attributes.AppFormVariable) eq 0>
    ...
    <cfset x = StructInsert(attributes.AppFormVariable,"insured_name",StructFind(BoatQuoteDetails,listGetAt(CONST_MQ_FirstName_FID,1,"|")) & " " & StructFind(BoatQuoteDetails,listGetAt(CONST_MQ_Surname_FID,1,"|")),"YES")>
    <cfif StructKeyExists(BoatQuoteDetails, listGetAt(CONST_MQ_Homephone_FID,1,"|"))>
        <cfset x = StructInsert(attributes.AppFormVariable,"insured_phone_home",StructFind(BoatQuoteDetails, listGetAt(CONST_MQ_Homephone_FID,1,"|")),"YES")>
    <cfelse>
    ...
    <cfif StructKeyExists(BoatQuoteDetails, listGetAt(CONST_MQ_MobilePhone_FID,1,"|"))>
        <cfset x = StructInsert(attributes.AppFormVariable,"insured_phone_mobile",StructFind(BoatQuoteDetails, listGetAt(CONST_MQ_MobilePhone_FID,1,"|")),"YES")>
    <cfelse>
    ...
    <cfloop list="#coverSelected#" index="aCover">
        ...
        <cfif getPremiumValue.recordCount gt 0 
            and (aCover eq CONST_MQ_Comp_LID OR aCover eq CONST_MQ_MotorOnly_LID
                OR aCover eq CONST_MQ_TPO_LID ) > <!--- OR aCover eq CONST_MQ_QuoteGapCover_LID --->
...
<cfif LOCAL_insurancePremium gt 1>
    ...
    select * from getExistingRates where marine_rateCategoryID = #CONST_ID_PayByMonthCharge# order by marine_rate_item_id
    </cfquery>
    <cfif LSIsNumeric(thePremium) and thePremium gt 1 and getSpecificRate.marine_rate_item_id neq "">
        ...
        <cfquery name="getSpecificRate" dbtype="query">
        select * from getExistingRates where marine_rateCategoryID = #CONST_ID_PayByMonthCharge_CCExtra# order by marine_rate_item_id
        </cfquery>
        <cfif LSIsNumeric(thePremium) and thePremium gt 1 and getSpecificRate.marine_rate_item_id neq "">
    ...
    <cfset tmpFile = "pdfResponse_marine_yminz_pbm_#attributes.formDataId#.pdf">
