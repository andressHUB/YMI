...
<cfif getFormData.recordCount gt 0>
    <cfwddx action="WDDX2CFML" input="#getFormData.xml_data#" output="theData">
     <cfif StructKeyExists(theData,ListFirst(CONST_MQ_QuoteStatus_FID,"|"))>
          <cfset x = StructUpdate(theData,ListFirst(CONST_MQ_QuoteStatus_FID,"|"),CONST_MQ_QuoteStatus_stage4_LID)>
     <cfelse>
          <cfset x = StructInsert(theData,ListFirst(CONST_MQ_QuoteStatus_FID,"|"),CONST_MQ_QuoteStatus_stage4_LID)>
     </cfif>
...
<cflocation url="adminMarine.cfm?act=quoteAdmin&fdid=#URL.fdid#" addtoken="no">
