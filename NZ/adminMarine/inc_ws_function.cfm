...
<cffunction name="updateWebServiceTempData" returntype="numeric">
    ...
    <cfelse>
        ...
        declare @next_id int
        set @next_id = (select isnull(max(ws_data_id),0)+1 from ymi_marine_ws_data)
    
        insert into ymi_marine_ws_data (ws_data_id, form_data_id, external_id, requester_id, data_type, last_updated, ws_data)
...
<cffunction name="createWebServiceLog" returntype="numeric">
    ...
    <cfargument name="marine_company_id" required="Yes" type="numeric">
    ...
    declare @next_id int
    set @next_id = (select isnull(max(ws_request_id),0)+1 from ymi_marine_ws_log)
    
    insert into ymi_marine_ws_log (ws_request_id, ip_addr, marine_company_id, requester_id, requestedAt, requestXML, external_id)
    values (@next_id, '#ARGUMENTS.ip_addr#',#ARGUMENTS.marine_company_id#,'#ARGUMENTS.requester_id#',#requestDT#,'#ARGUMENTS.requestXML#','#ARGUMENTS.external_id#')
...
<id>
    ...
    <extDealer id="D987">ROCKAFELLER MARINE LTD</extDealer>
...
<cffunction name="getThirdgenDetails" returntype="struct">
    ...
    <cfif StructKeyExists(detailStruct,"extUserID") and StructKeyExists(detailStruct,"extDealerID")
        ...
        SELECT top 1 ud.user_data_id, ud.user_name, ud.user_email, c.default_tree_node_id, 
            fhd.#ListLast(CONST_WS_ExtUser_FID,"|")#, utn.tree_node_id, fhd2.#ListLast(CONST_WS_ExtDealership_FID,"|")#
        ...
        INNER JOIN thirdgen_user_role ur on ud.user_data_id = ur.user_data_id and ur.role_id = #CONST_MARINE_ROLEID#
...
<cffunction name="createXMLPackage" returntype="string">
    ...
    <cfif not IsDefined("CONST_WS_ClientID")>
        <cfinclude template="../adminMarine/constants_ws.cfm">
    </cfif>
...
<cffunction name="encryptDecryptString" returntype="string">
    ...
    <cfargument name="sharedKey" required="No" type="string" default="YMINZ15">
